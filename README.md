...
The default `extended` profile preserves the FDT XML generated by previous versions.
To generate FDT XML conforming to the profiled schema in 3GPP TS 26.346 clause L.6.1,
select `ts-126-346-l6` explicitly:

```bash
flute-sender --fdt-xml-profile ts-126-346-l6 file1.txt
```

Rust callers can set `sender::Config::fdt_xml_profile` to
`sender::FdtXmlProfile::Ts26346L6`. Python callers can assign
`"ts-126-346-l6"` to `sender.Config.fdt_xml_profile`.
